namespace VoicevoxEngineSharp.Core.Acoustic.Native
{
    internal class EachCppForwarder
    {
        public static bool Initialize(
            ...
            bool use_gpu) => EachCppForwarderUnmanaged.initialize(yukarin_s_forwarder_path, yukarin_sa_forwarder_path, decode_forwarder_path, use_gpu);

        public static float[] YukarinSForward(int length, long[] phoneme_list, long[] speaker_id)
        {
            var output = new float[length];
            EachCppForwarderUnmanaged.yukarin_s_forward(length, phoneme_list, speaker_id, output);
            return output;
        }
        ...
        {
            var output = new float[length];
            EachCppForwarderUnmanaged.yukarin_sa_forward(
        ...
        public static float[] DecodeForward(int length, int phoneme_size, float[] f0, float[] phoneme, long[] speaker_id)
        {
            ...
            EachCppForwarderUnmanaged.decode_forward(length, phoneme_size, f0, phoneme, speaker_id, output);
            return output;
        }
    }
}
